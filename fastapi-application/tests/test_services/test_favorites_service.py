...
async def test_create_favorite(
    ...
    """
    service = FavoritesService(test_session)
    favorite_data = FavoriteCreate(
        user_id=test_user.id,
        product_id=test_product.id,
    )
    favorite = await service.create_favorite(favorite_data)
...
async def test_get_favorites(
    ...
    result = await service.get_favorites(test_user.id)
...
async def test_delete_favorite(
    ...
    await service.delete_favorite_by_id(favorite.id)
